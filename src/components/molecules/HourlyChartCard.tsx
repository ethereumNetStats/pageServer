// HourlyChart.tsxからデータ名と集計データを受け取って、データ名とチャートを対応させて表示する
...
type Props = {
    dataName: string,
    hourlyNetStats: netStatsArray,
};

// react-chartjs-2用の登録
ChartJS.register(...registerables);

// コンポーネント'HourlyChartCard'の定義
export const HourlyChartCard: VFC<Props> = memo(({dataName, hourlyNetStats}) => {
        ...
        hourlyNetStats.forEach((obj: netStatsString) => {
    ...
    , (prev, next) => {
        if (prev.hourlyNetStats.length !== 0) {
            return prev.hourlyNetStats[prev.hourlyNetStats.length - 1].endTimeUnix === next.hourlyNetStats[next.hourlyNetStats.length - 1].endTimeUnix
        } else {
            return false
        }
    });
