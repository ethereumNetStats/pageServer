...
type Props = {
    dataName: string;
    weeklyBasicData: recordOfEthDBArray;
};

ChartJS.register(...registerables);

export const WeeklyChartCard: VFC<Props> = memo(({dataName, weeklyBasicData}) => {
        ...
        weeklyBasicData.forEach((obj: recordOfEthDB) => {
    ...
    , (prev, next) => {
        if (prev.weeklyBasicData.length !== 0) {
            return prev.weeklyBasicData[prev.weeklyBasicData.length - 1].endTimeUnix === next.weeklyBasicData[next.weeklyBasicData.length - 1].endTimeUnix
        } else {
            return false
        }
    });
